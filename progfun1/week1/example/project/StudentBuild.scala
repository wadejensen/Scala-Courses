...
/**
  ...
  */
object StudentBuild extends CommonBuild {
  ...
  ).settings(packageSubmissionZipSettings)
  ...
  val packageSubmissionZipSettings = Seq(
    ...
    mappings in (Compile, packageBin) := {
      val relativePaths =
        (unmanagedResources in Compile).value.flatMap(relativeTo((unmanagedResourceDirectories in Compile).value)(_))
